    ...
    async def stats(self, ctx, amount=1):
        ...
        try:
            async with ClientSession() as session:
                async with session.get("https://coinlib.io/api/v1/coin?key=593ce9b1bef849c6&pref=BTC&symbol=QUAD", headers=headers) as response:
                    responseRaw = await response.read()
                    priceData = json.loads(responseRaw)
                    for item in priceData:
                        embed= discord.Embed(colour=0x00FF00)
                        embed.set_author(name='QUAD Information', icon_url="https://i0.wp.com/quadrivaecoin.com/wp-content/uploads/2018/07/quadrivae-128-1.png")
                        embed.add_field(name="Price (BTC)", value="${}".format(item['price']))                    
                        embed.set_footer(text="https://wallet.crypto-bridge.org/market/BRIDGE.QUAD_BRIDGE.BTC", icon_url="https://i0.wp.com/quadrivaecoin.com/wp-content/uploads/2018/07/quadrivae-128-1.png")
