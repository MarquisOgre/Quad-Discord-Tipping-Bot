...
class Tip:
    ...
    async def tip(self, ctx, user:discord.Member, amount:float):
        ...
        if float(balance) < amount:
            await self.bot.say("{} **:warning:You cannot tip more QUAD than you have!:warning:**".format(ctx.message.author.mention))
        else:
            mysql.add_tip(snowflake, tip_user, amount)
            await self.bot.say("{} **Tipped {} {} QUAD! <:quad:469787831137599488>**".format(ctx.message.author.mention, user.mention, str(amount)))
